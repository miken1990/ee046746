...
def image_seq_to_video(imgs_path, output_path='./video.mp4', fps=15.0):
    ...
    out = cv2.VideoWriter(output, fourcc, fps, image_seq_to_video_changed, size)
...
def image_seq_to_video_changed(imgs_path, num_images, output_path='./video.mp4', fps=15.0):
    output = output_path
    img_array = []
    for i in range(num_images):
        filename = str(i+100) + '.jpg'
        filename = os.path.join(imgs_path, filename)
        img = cv2.imread(filename)
        height, width, layers = img.shape
        # img = cv2.resize(img, (width // 2, height // 2))
        img = cv2.resize(img, (width, height))
